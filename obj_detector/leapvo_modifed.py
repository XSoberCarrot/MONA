...
class LEAPVO:
    ...
    def get_window_trajs(self, only_coords=False):
        ...
        if "VIS_THRESHOLD" in self.cfg.slam:
            ...
            # ===================================================
            vis_label_dyn = vis_e > self.cfg.slam.VIS_THRESHOLD
            # ===================================================
        else:
            vis_label = torch.ones_like(vis_e) > 0
            # ===================================================
            vis_label_dyn = torch.ones_like(vis_e) > 0
            # ===================================================
        ...
        if "dynamic_e" in stats and "STATIC_THRESHOLD" in self.cfg.slam:
            ...
            # ===================================================
            dynamic_label = statie_e < static_th
            vis_label_dyn = vis_label_dyn & dynamic_label
            # ===================================================
        ...
        if "var_e" in stats and "CONF_THRESHOLD" in self.cfg.slam:
            ...
            # ===================================================
            vis_label_dyn = vis_label_dyn & conf_label
            # ===================================================

        local_target = local_target[:, :S_local]
        vis_label = vis_label[:, :S_local]
        # ===================================================
        vis_label_dyn = vis_label_dyn[:, :S_local]
        # ===================================================

        # ===================================================
        # print("\nvis_label shape: ", vis_label.shape)
        # print("vis_label values: ", vis_label)

        # print("static_label shape: ", static_label.shape)
        # print("static_label values: ", static_label)

        # print("dynamic_e shape: ", stats["dynamic_e"].shape)
        # print("dynamic_e values: ", stats["dynamic_e"])

        # print("local_target shape: ", local_target.shape)
        # print("local_target values: ", local_target)

        # print("vis_label_dyn shape: ", vis_label_dyn.shape)
        # print("vis_label_dyn values: ", vis_label_dyn)

        # print("I am used")
        # ===================================================

        # ==================================================================
        # Save the dynamic coordinates
        rounded_coords = torch.round(local_target[0,-1,:,:]).long()
        vis_label_dyn = vis_label_dyn[0,-1,:]
        dynamic_coords = rounded_coords[vis_label_dyn]
        # print("dynamic_coords shape: ", dynamic_coords.shape)
        # print("dynamic_coords values: ", dynamic_coords)
        
        # Save into json file
        print(f"Curr frame: {self.current_frame}")
        pwd = os.getcwd()
        dyn_coords_dir = f"{pwd}/{self.cfg.data.savedir}/{self.cfg.data.name}/dynamic_points"
        if not os.path.exists(dyn_coords_dir):
            os.makedirs(dyn_coords_dir)
        dyn_coords_path = f"{dyn_coords_dir}/frame_{self.current_frame}.json"
        dynamic_coords = dynamic_coords.cpu().numpy()
        dynamic_coords = dynamic_coords.tolist()
        with open(dyn_coords_path, 'w') as f:
            json.dump(dynamic_coords, f)
        print(f"Saved dynamic coordinates to {dyn_coords_path}")
        # ==================================================================
