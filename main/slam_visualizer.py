...
class LEAPVisualizer(SLAMVisualizer):
    ...
    def draw_tracks_on_video(
        ...
        # ========================================================
        # dynamic_points = []
        # # If the vector_colors is mpl.colors.to_rgba("red") or "yellow", then it is dynamic, set dynamic_flag to True
        # if vector_colors[0, 0, 0] == 1.0 and vector_colors[0, 0, 1] == 0.0 and vector_colors[0, 0, 2] == 0.0:
        #     dynamic_flag = True
        # elif vector_colors[0, 0, 0] == 1.0 and vector_colors[0, 0, 1] == 1.0 and vector_colors[0, 0, 2] == 0.0:
        #     dynamic_flag = True
        # if dynamic_flag:
        #     print("Dynamic track")
        # else:
        #     print("Static track")
        # dynamic_flag = False
        # print(F"Vector colors: {vector_colors.shape}")
        # if vector_colors is not None and vector_colors.shape[0] !=0 and vector_colors.shape[1] != 0:
        #     print(f"Value of vector_colors: {vector_colors[0, 0]}")
        # else:
        #     print("Vector colors is empty")

        # if vector_colors is not None and vector_colors.shape[0] !=0 and vector_colors.shape[1] != 0:
        #     if vector_colors[0, 0, 0] == 255. and vector_colors[0, 0, 1] == 0.0 and vector_colors[0, 0, 2] == 0.0:
        #         dynamic_flag = True
        #         print("Dynamic track")
        ...
        # ========================================================                
        #     # Save dynamic points for the current frame
        #     print(f"Frame {t}: {len(frame_points)} dynamic points")
        #     frame_points_for_json = [[int(x), int(y)] for x, y in frame_points]  # Convert to list of lists with int
        #     dynamic_points.append({
        #         "frame_id": int(t),  # Convert to int
        #         "dynamic_points": frame_points_for_json
        #     })
        # ========================================================
        # ========================================================
        # # Save dynamic points to a JSON file
        # dynamic_points_path = os.path.join(self.save_dir, "dynamic_points.json")
        # with open(dynamic_points_path, "w") as f:
        #     json.dump(dynamic_points, f)
        # print(f"Dynamic points saved to {dynamic_points_path} successfully")
        # ========================================================
    ...
    def draw_tracks_on_frames(self):
        ...
        # # Save self.frames to seperate png files
        # for i, frame in enumerate(self.frames):
        #     # Ensure the frame has 3 dimensions (C, H, W)
        #     if frame.dim() == 3:
        #         frame = frame.permute(1, 2, 0).byte().cpu().numpy()  # Convert to HWC format
        #     elif frame.dim() == 4:
        #         frame = frame[0].permute(1, 2, 0).byte().cpu().numpy()  # Convert to HWC format
        #     else:
        #         raise RuntimeError(f"Unexpected frame dimensions: {frame.dim()}")
        ...
        for t, track in enumerate(self.tracks):
            ...
            # ========================================================
            # # Save dynamic points (2D coordinates)
            # dyn_tracks_2d = dyn_tracks[0].long().detach().cpu().numpy()  # S, N, 2
            # dynamic_points_2d = dyn_tracks_2d[..., :2].tolist()
            # print(f"Frame {fid}: {len(dynamic_points_2d)} dynamic points")
            # dynamic_points.append({
            #     "frame_id": fid,
            #     "dynamic_points": dynamic_points_2d if dyn_tracks_2d.size > 0 else []
            # })
            # ========================================================
        ...
        # with open(dynamic_points_path, "w") as f:
        #     json.dump(dynamic_points, f)
        # print(f"Dynamicx  points saved to {dynamic_points_path} success")

        # # Save res_video_dyn frame by frame
        # dyn_frames_dir = os.path.join(self.save_dir, "res_video_dyn_frames")
        # os.makedirs(dyn_frames_dir, exist_ok=True)
        # for i, frame in enumerate(res_video_dyn):
        #     frame = frame.permute(1, 2, 0).byte().cpu().numpy()  # Convert to HWC format
        #     frame_path = os.path.join(dyn_frames_dir, f"res_video_dyn_frame_{i:04d}.png")
        #     cv2.imwrite(frame_path, frame)
        #     print(f"Saved res_video_dyn frame {i} to {frame_path}")
        # ========================================================
    ...
    def save_video(self, filename, writer=None, step=0):
        ...
        if writer is not None:
            ...
            print(f"Video saved to {save_path} success")
